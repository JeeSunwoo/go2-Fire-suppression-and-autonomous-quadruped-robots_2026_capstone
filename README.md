<div align="center">
  <img width="100" height="100" alt="image" src="https://github.com/user-attachments/assets/e77cd3aa-2df5-48d4-8e23-a952c47276be" />
...
AreaTechnologyMiddlewareROS2 Foxy, Cyclone DDSNavigationNav2, SLAM ToolboxPerceptionIntel RealSense D435i, Hesai XT-16Decision (VLM)VLFM powered by the Anthropic Claude APIRobot ControlUnitree Sport APIHardwareUnitree Go2, NVIDIA JetsonProgrammingPython, C++, ROS2
...
| Package                        | Description                                                                                                      |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| **run_vlm_stack.sh**           | One-shot launcher for the complete robot software stack.                                                         |
| **vlm/vlfm_source_nav_v20.py** | Main VLFM navigation node that performs VLM inference and autonomous navigation toward the detected fire source. |
| **my_go2_controller**          | Go2 control package containing teleoperation, safety monitoring, and obstacle-gap detection.                     |
| **my_go2_nav_bringup**         | Launch files and configuration for SLAM Toolbox, Nav2, and the `cmd_vel` → Unitree Sport API bridge.             |
| **my_go2_odom_relay**          | Relays the Go2 internal odometry to `/odom` while publishing the required TF frames.                             |
| **vlfm_nav**                   | ROS2 package containing the VLFM navigation node.                                                                |
| **HesaiLidar_ROS_2.0**         | External repository providing the Hesai XT-16 ROS2 driver.                                                       |
| **realsense-ros**              | External repository providing Intel RealSense ROS2 drivers.                                                      |
| **unitree_ros2**               | External repository providing the Unitree Go2 ROS2 SDK and interfaces.                                           |

---

## External Dependencies

This repository contains only the project-specific source code.

Before building, clone the required third-party repositories into the `src` directory.
...
| Decision      | Vision-Language Frontier Map (VLFM) using the Anthropic Claude API |
...
After cloning the external repositories, build the ROS2 workspace.
...
### 1. Launch the complete robot stack
...
### 2. Start the VLFM navigation node

Open another terminal.

```bash
source /opt/ros/foxy/setup.bash
source ~/go2/install/setup.bash
...
Press **Ctrl+C** in the terminal running `run_vlm_stack.sh`.

All launched ROS2 processes are terminated automatically, and logs are saved under `run_logs/`.

---

## License

This repository contains the original project implementation developed by **Team R.O.I, Kwangwoon University**.

The following packages are maintained by their original authors and are distributed under their respective licenses:

* Unitree ROS2
* HesaiLidar_ROS_2.0
* Intel RealSense ROS

Please refer to each repository for detailed license information.
...
© 2026 Team R.O.I. All Rights Reserved.
